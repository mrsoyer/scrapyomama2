...
{
	public function executeGetAPI($api_url,$proxy)
	{
        $ch = curl_init();
		if (!empty($proxy))
        {
	...
	public function executePutAPI($api_url, $params,$proxy)
	{
    ...
    public function executeDeleteAPI($api_url, $params,$proxy)
	{
        $ch = curl_init();
	...
	public function executePostAPI($api_url, $params,$proxy)
	{
	    $curl_handler = curl_init();
		if (!empty($proxy))
        {
            curl_setopt($curl_handler, CURLOPT_PROXY, $proxy);
           // curl_setopt($curl, CURLOPT_PROXYUSERPWD, self::$proxyAuth);
        }
	...
	public function executePostJsonAPI($api_url, $params,$proxy)
	{
		return $this->parseJSON($this->executePostAPI($api_url, $params,$proxy));
	}

	public function executeGetJsonAPI($api_url, $params = [],$proxy)
	{
		return $this->parseJSON($this->executeGetAPI($api_url.$this->parseGetParams($params),$proxy));
	}

	public function executePutJsonAPI($api_url, $params,$proxy)
	{
		return $this->parseJSON($this->executePutAPI($api_url, $params,$proxy));
	}

	public function executeDeleteJsonAPI($api_url, $params,$proxy)
	{
		return $this->parseJSON($this->executeDeleteAPI($api_url, $params,$proxy));
	}
